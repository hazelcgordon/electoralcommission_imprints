...
kable(recall_table, format = "html", digits = 2, caption = "Recall of campaigner name") %>%
...
ggplot(preds_recall, aes(x = x, y = predicted, group = 1)) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), width = 0.1, color = "sienna") +
...
model <- lmer(outcome ~ Training.condition + version + Training.condition*version + (1|id) + (1|advert))
...
fit_trust_model <- function(data, outcomes) {
  ...
  for (outcome in outcomes) {
    formula <- as.formula(paste(outcome, "~ Training.condition + version + Training.condition*version + (1|id) + (1|advert)", sep = ""))
...
pred.labels = c("Intercept", "Training", "Digital Imprint included", "Training*imprint"), dv.labels = "Perceived trustworthiness")

tab_model(model_believable_value, file = "tables/believe.html",  
pred.labels = c("Intercept", "Training", "Digital Imprint included", "Training*imprint"), dv.labels = "Perceived believability")

tab_model(model_accurate_value, file = "tables/accurate.html",  
pred.labels = c("Intercept", "Training", "Digital Imprint included", "Training*imprint"), dv.labels = "Perceived accuracy")

tab_model(model_factual_value, file = "tables/factual.html",  
pred.labels = c("Intercept", "Training", "Digital Imprint included", "Training*imprint"), dv.labels = "Perceived fact versus opinion")
